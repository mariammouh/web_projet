...
@section('content')
    <div class="container-fluid py-4">
          ...
          {{-- Dans la section de chaque utilisateur --}}
@foreach($users as $user)
  <li class="list-group-item border-0 d-flex p-4 mb-2 bg-gray-100 border-radius-lg">
    {{-- ... Autres éléments ... --}}
    <div class="ms-auto text-end">

      {{-- Bouton Commentaires (AJOUTÉ) --}}
      <a class="btn btn-link text-info px-3 mb-0" data-bs-toggle="collapse" href="#comments-{{ $user->id }}">
  ...
  <div class="collapse" id="comments-{{ $user->id }}">
    <div class="card card-body bg-light mt-2">
      <h6 class="text-info">📝 Commentaires de {{ $user->name }} :</h6>
      @if($user->comments->count() > 0)
        @foreach($user->comments as $comment)
          <div class="list-group-item mb-3 p-3 border rounded">
            <p class="mb-2"><strong>Contenu :</strong> {{ $comment->content }}</p>
            <div class="d-flex gap-2 mb-2">
              @if($comment->rating)
                <span class="badge bg-primary">Note : {{ $comment->rating }}/5</span>
              @endif
              <span class="badge bg-danger">Signalements : {{ $comment->reports_count }}</span>
            </div>
            @if($comment->film)
              <a href="{{ route('film.details', $comment->film_id) }}" class="text-success">
                🎬 {{ $comment->film->title }}
              </a>
            @elseif($comment->show)
              <a href="{{ route('show.details', $comment->show_id) }}" class="text-success">
                📺 {{ $comment->show->title }}
              </a>
            @endif
...
@endforeach {{-- Ne pas oublier de fermer la boucle ici --}}
        </div>
        <div class="card mt-4">
